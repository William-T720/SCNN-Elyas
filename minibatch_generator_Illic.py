...
class myDataGeneratorAug(object):
    ...
    def __data_generation(self, labels, list_IDs_temp, seed, datagenargs):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            # print(i)
            # print(ID[1][0])
            img1 = mpimg.imread(ID[1][0])[:610,:]
#            img1 = cv2.imread(ID[1][0])[:610,:]
#            img1a = cv2.cvtColor(img1,cv2.COLOR_BGR2RGB)
            ...
            #print(x_lefta.shape)
            x_leftb = cv2.resize(x_lefta[0, :], (self.dim_x, self.dim_y))
            #x_leftb=preprocess_input(x=np.expand_dims(x_leftb.astype(float), axis=0))[0]
            X1.append(x_leftb)

            img2 = mpimg.imread(ID[1][1])[:610,:]
#            img2 = cv2.imread(ID[1][1])[:610,:]
#            img2a = cv2.cvtColor(img2,cv2.COLOR_BGR2RGB)
            ...
            x_rightb = cv2.resize(x_righta[0, :], (self.dim_x, self.dim_y))
